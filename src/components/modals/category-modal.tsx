...
export function CategoryModal({ isOpen, onClose, onSuccess, category }: CategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (category) {
        await api.put(`/cms/categories/${category.id}`, formData);
      ...
      >
        <div className="p-8 pb-0 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Phân loại nội dung hệ thống</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Tên danh mục</label>
              <input
                required
                value={formData.name}
                ...
                placeholder="VD: Kinh nghiệm học tập"
              />
            </div>
          ...
          <div className="pt-6 border-t border-slate-50 flex gap-3">
             ...
             >
               {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : category ? 'CẬP NHẬT' : 'TẠO MỚI'}
             </Button>
          </div>
        </form>
      </motion.div>
